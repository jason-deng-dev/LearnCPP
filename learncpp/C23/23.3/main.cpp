...
public:
	...
	{
		m_teachers.push_back(std::reference_wrapper<const Teacher>(teacher));
	}

	friend std::ostream& operator<<(std::ostream& out, const Department& dep)
	{
		std::cout << "This department has the following teachers ";
		for (const Teacher teacher: dep.m_teachers)
		{
